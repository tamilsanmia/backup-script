...
MYSQL_PORT="3306"
MYSQL_USER="faveo"
MYSQL_PASSWORD="ftWJO004Cm23fyCL"
...
File_System_Backup() {
  ...
  sshpass -p "$REMOTE_PASSWORD" scp -P "$REMOTE_PORT" "$BACKUP_FILE" "$REMOTE_USER"@"$REMOTE_HOST":"$REMOTE_DIRECTORY" || { echo "Error: Failed to transfer backup file to remote server"; exit 1; }
}
...
MySQL_Backup() {
  ...
  # Transfer backup file to remote server
  sshpass -p "$REMOTE_PASSWORD" scp -P "$REMOTE_PORT" "$MYSQL_BACKUP_FILE.tar.gz" "$REMOTE_USER"@"$REMOTE_HOST":"$REMOTE_DIRECTORY" || { echo "Error: Failed to transfer backup file to remote server"; exit 1; }
}
